...
export default function HowItWorksSection({ lang = "EN" }) {
  const tr = mkTr(lang);
  return (
    <section
      id="home-how"
    ...
    >
      ...
      <div className="hiw-wrap">
        <div className="hiw-head">
          <div className="hiw-eyebrow"><span className="hiw-dot" /> {tr("Процес", "The process")}</div>
          <h2 className="hiw-title">{tr("Три кроки до ", "Three steps to ")}<em>{tr("впевненості.", "confident.")}</em></h2>
          <p className="hiw-sub">{tr("Жодних 47 фільтрів-випадайок. Жодної реклами. Жодного тиску. Лише чесні поради від AI, створеного знаходити правильне авто — а не найдорожче.", "No filtering through 47 dropdowns. No ads. No pressure. Just honest guidance from an AI built to find the right car — not the most expensive one.")}</p>
        </div>
        ...
        <div className="hiw-grid">
          <div className="hiw-step">
            ...
            <div className="hiw-t">{tr("Спілкування з радником", "Chat with the advisor")}</div>
            <p className="hiw-b">{tr("Опишіть, що вам потрібно, звичайними словами — бюджет, сценарій, розмір сім'ї, як їздите. Радник ставить справжні уточнювальні питання, як обізнаний друг.", "Describe what you need in plain language — budget, use case, family size, how you drive. The advisor asks real follow-up questions, like a knowledgeable friend.")}</p>
            <div className="hiw-ex">
              <p style={{ fontSize: "12.5px", color: "#cdc6bc", lineHeight: 1.5, margin: 0 }}>
                <span style={{ color: "var(--muted)" }}>{tr("Ви:", "You:")}</span> {tr("«Двоє дітей, собака, вихідні за містом, до €35k»", "“Two kids, one dog, weekend B-roads, under €35k”")}<br />
                <span style={{ color: "var(--amber)" }}>{tr("Радник:", "Advisor:")}</span> {tr("«Що важливіше — обʼєм багажника чи драйв?»", "“Do you prioritise boot space or driving feel?”")}
              </p>
            </div>
          </div>
          ...
          <div className="hiw-step">
            ...
            <div className="hiw-t">{tr("Розумні поради", "Smart recommendations")}</div>
            <p className="hiw-b">{tr("AI показує авто, що відповідають вашому опису — оцінені за відповідністю, а не за спонсорством. Кожен варіант має відсоток впевненості й пояснення чому.", "The AI surfaces cars matched to what you described — scored by fit, not by sponsorship. Each match comes with a confidence percentage and a reason why.")}</p>
            <div className="hiw-ex">
              ...
              <p style={{ fontSize: "11.5px", color: "var(--muted)", marginTop: 7, lineHeight: 1.4, marginBottom: 0 }}>{tr("Універсал + задні сидіння + траси + у межах бюджету", "Estate + rear seats + B-roads + within budget")}</p>
            </div>
          </div>
            ...
            <div className="hiw-ex">
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "12.5px" }}>
                  <span style={{ color: "#cdc6bc" }}>{tr("Дилер · Амстердам", "Dealer · Amsterdam")}</span>
                  <span style={{ fontFamily: "'Fraunces',Georgia,serif", color: "var(--amber)" }}>€28,400</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "12.5px" }}>
                  <span style={{ color: "#cdc6bc" }}>{tr("Приватний · Антверпен", "Private · Antwerp")}</span>
